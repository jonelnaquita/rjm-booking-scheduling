            ...
            <div class="main-panel">
                <div class="content-wrapper">
                    ...
                    <div class="row" style="margin-bottom: 20px;">
                        ...
                        <div class="col-auto">
                            <button type="button" class="btn btn-primary btn-rounded btn-icon-text"
                                data-bs-toggle="modal" data-bs-target="#schedule-modal">
                    ...
                    <p class="text-muted mt-2">
                        ...
                        <span class="highlight-red px-1">Highlighted schedules</span> indicate a conflict with another
                        schedule.
                    </p>
                    ...
                    <div class="table-responsive">
                        <table id="schedule-table" class="table table-hover"
                            ...
                            <tbody>
                                <?php include '../api/schedule/fetch-schedule-table.php'; ?>
                                <!-- Adjust the path to where you save this PHP code -->
                            </tbody>
                        </table>
                    </div>
                </div>
...
<script>
    $(document).ready(function () {
        ...
        // Get terminal_id from a variable or another source (e.g., from a hidden input or global JS variable)
        var terminal_id = <?php echo $terminal; ?>;  // Pass the PHP variable directly

        $.ajax({
            url: '../api/schedule/fetch-destination-from.php',
            type: 'GET',
            dataType: 'json',
            data: { terminal_id: terminal_id },  // Send terminal_id to the server
...
<script>
    $(document).ready(function () {
        $('.save-schedule').on('click', function () {
            ...
            let isNewScheduleAdded = false; // Track if any new schedule was added
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        if (response === 'Schedule already exists.') {
                            ...
                            isNewScheduleAdded = true; // Mark that at least one new schedule was added
                        }
                    },
                    error: function () {
            ...
            setTimeout(function () {
                ...
                // Send email if a new schedule was added
                if (isNewScheduleAdded) {
        ...
        function sendEmailToStaff(bus_id, from_id, to_id, formatted_dates, departure_time) {
            console.log("Bus ID:", bus_id); // Log bus_id before the AJAX request
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.email) {
                        console.log("Staff email to be sent:", response.email); // Log the email to be sent
                        ...
                        $.ajax({
                            ...
                            success: function (emailResponse) {
                                if (emailResponse.success) {
                                    ...
                                    toastr.error('Error: ' + emailResponse.error);
                                }
                            },
                            error: function (xhr, status, error) {
                                toastr.error('Error sending email to staff: ' + error);
                            }
                        });
                    } else {
                        toastr.info('No staff email found for this bus.');
                    }
                },
                error: function (xhr, status, error) {
                    toastr.error('Error retrieving staff email: ' + error);
                }
            });
        }
    });
